...
type OrderServiceRawEntity = Omit<
  OrderServiceModel,
  'orderId' | 'serviceSnapshotId'
> & {
  serviceSnapshot: ServiceSnapshotModel;
};

function rawEntityToEntity(
  rawOrderService: OrderServiceRawEntity,
): OrderServiceEntity {
  const { serviceSnapshot, ...restOfOrderServiceFields } = rawOrderService;
  const { originalServiceId, ...serviceSnapshotWithoutOriginalServiceId } =
    serviceSnapshot;
...
export class OrderServicesService {
  ...
  async create(
    ...
    const [createdOrderService] = await this.createMany(orderId, [
  ...
  async createMany(
    orderId: string,
    createOrderServiceDtos: CreateOrderServiceDto[],
  ): Promise<OrderServiceEntity[]> {
    const createdOrderServices = await Promise.all(
      createOrderServiceDtos.map(async (createOrderServiceDto) => {
        const serviceSnapshotId =
          createOrderServiceDto.serviceSnapshot?.id ??
          (await this.serviceSnapshotsService.pickLatestSnapshotOf(
            createOrderServiceDto.serviceId,
          ));

        const createdOrderService =
          await this.currentTransaction.orderService.create({
            data: {
  ...
  async findMany(
    ...
    const rawOrderServices =
      await this.currentTransaction.orderService.findMany({
        where: {
          orderId,
        },
        ...selectOrderServiceEntityFields,
  ...
  async findOne(
    orderId: string,
    serviceId: string,
  ): Promise<OrderServiceEntity | null> {
    const rawOrderService =
      await this.currentTransaction.orderService.findFirst({
        where: {
          orderId,
          serviceSnapshot: {
  ...
  async update(
    ...
    if (updateOrderServiceDto.serviceId) {
      ...
      newServiceSnapshotId ??=
        await this.serviceSnapshotsService.pickLatestSnapshotOf(
          updateOrderServiceDto.serviceId,
        );
    }
    ...
    const updatedOrderService = (await this.findOne(
      orderId,
      newServiceId,
    )) as OrderServiceEntity;
