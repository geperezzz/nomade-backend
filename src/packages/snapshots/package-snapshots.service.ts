...
export class PackageSnapshotsService {
  ...
  async pickLatestSnapshotOf(packageId: string): Promise<string> {
    ...
    const latestSnapshot =
      await this.currentTransaction.packageSnapshot.findFirst({
        where: {
    ...
    if (
      !latestSnapshot ||
      latestSnapshot.snapshotTimestamp < originalPackage.lastUpdateTimestamp
    ) {
  ...
  async createSnapshotOf(packageId: string): Promise<string> {
    ...
    const containedServicesAsSnapshots = await Promise.all(
      originalPackage.containedServices.map(
        async ({ service, amountContained }) => ({
          serviceId: await this.serviceSnapshotsService.createSnapshotOf(
            service.id,
          ),
          amountContained,
        }),
    ...
    const { id: snapshotId } =
      await this.currentTransaction.packageSnapshot.create({
        data: {
